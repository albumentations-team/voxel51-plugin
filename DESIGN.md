...
- Владелец результата: Андрей Б.
- Способ реализации: Андрей ведёт проект вместе с coding agents; он может читать, проверять и писать код, но не обязан реализовывать весь плагин лично.
...
Работа считается законченной, когда Андрей проверил код в выбранном им объёме и может:
...
## Как Андрей работает с этим документом

Андрей — программист и владелец результата. Он может читать diff, проверять технические решения, исправлять код и самостоятельно реализовывать любые части плагина. Coding agents берут на себя тот объём реализации и рутинных проверок, который Андрей решит им поручить. От Андрея не требуется лично написать весь плагин с нуля.

Технические разделы ниже — контракт для всех участников разработки. Они объясняют, что нужно построить, какие данные нельзя испортить и какими тестами доказать результат. Андрей использует четыре вида доказательств:
...
Андрей также проверяет код сам с той глубиной, которую считает нужной. Независимая проверка review agent дополняет его проверку и помогает не пропустить ошибку.

Знание нейросетей для приёмки первой версии не требуется. Андрею достаточно понимать пользовательский сценарий, ожидаемый результат и ограничения. Сложные термины определены рядом и в [словаре](#словарь).

## Как вести разработку вместе с агентами

Один pull request — одна ограниченная задача. Если реализацию выполняет implementation agent, Андрей передаёт ему:
...
Андрей может сам реализовать часть или весь такой PR, внести правки после агента или поручить реализацию целиком. Разбиение на небольшие PR остаётся тем же: оно упрощает тестирование и независимую проверку независимо от автора кода.
...
Андрей принимает PR, когда CI проходит, демонстрация воспроизводится по README, review agent не нашёл блокирующих проблем, а исходные изображения и разметка остались неизменными. Перед принятием Андрей сам проверяет код в удобном ему объёме: может прочитать весь diff, сосредоточиться на рискованных местах или внести собственные изменения.
...
Обычно этап выполняет implementation agent и проверяет отдельный review agent. Андрей может написать или переписать любую часть сам и участвует в проверке кода. Этап заканчивается тестами и демонстрацией, которую Андрей может повторить по README. Следующий PR начинается после принятия предыдущего. Так ошибка не переезжает во все следующие задачи.
...
Этот раздел предназначен для каждого, кто реализует или проверяет этап: Андрея, implementation agent и review agent. Андрей выбирает, какие материалы изучить самому, а какие передать агентам вместе с задачей на PR.
...
### Необязательный ML-контекст для Андрея
