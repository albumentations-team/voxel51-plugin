...
Integration branch for active development. Feature branches start from `dev`
and merge back into `dev` by pull request.

`feature/*`

Short-lived task branches. Each branch should cover one coherent change and one
...
For the first release, `vX.Y.Z` is `v0.1.0`.
...
- `release/v0.1.0` for the first release;
