...
- `Keypoints`: `Keypoint.points` are converted from relative FiftyOne
  ...
  points.
- `Segmentation`: in-memory `Segmentation(mask=...)` masks are passed through
  Albumentations `masks` and written to the output sample.
...
classes, external mask-path workflows, detection instance masks, polylines,
custom embedded documents, video labels, 3D labels, and transform-specific
target requirements should be added in follow-up tasks with focused tests.
